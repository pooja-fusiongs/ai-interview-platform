...
Aggregates per-question scores into an overall score (1-10),
...
def generate_recommendation(
    ...
    high_scores = sum(1 for a in answer_scores if a.get("score", 0) >= 7.5)
    if high_scores > 0:
        strengths.append(f"Scored above 7.5 on {high_scores} out of {total} questions")
    ...
    low_scores = sum(1 for a in answer_scores if a.get("score", 0) < 4)
    if low_scores > 0:
        weaknesses.append(f"Scored below 4.0 on {low_scores} out of {total} questions")
