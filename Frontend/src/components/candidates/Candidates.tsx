...
const Candidates = () => {
  ...
  // Remove unused variables comment
  ...
  const handleGenerateQuestions = async (candidate: Candidate) => {
    // For general candidates page, we don't have a specific job context
    // This function might need to be updated based on your requirements
    toast.error(`Please select a specific job to generate questions for ${candidate.name}`)
    return
    
    // Alternative: If you want to allow question generation without a specific job:
    // try {
    //   setGeneratingQuestions(candidate.id)
    //   await questionGenerationService.generateQuestions({
    //     job_id: 0, // or some default job ID
    //     candidate_id: candidate.id,
    //     generation_mode: 'preview'
    //   })
    //   toast.success(`Questions generated successfully for ${candidate.name}! (Preview Mode)`)
    // } catch (error) {
  ...
  const renderCandidates = () => (
    <Box sx={{ padding: '20px', background: '#f8fafc', height: '100%' }}>
      ...
      {/* Candidates Grid */}
        ...
        <Dialog open={isProfileOpen} onClose={handleCloseProfile} maxWidth="sm" fullWidth>
          ...
          <DialogContent dividers sx={{ background: '#fff' }}>
            <Box sx={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '16px' }}>
              <Avatar sx={{ width: 64, height: 64 }} src={`/api/placeholder/64/64`} alt={menuCandidate?.name || 'Candidate'} />
              <Box sx={{ flex: 1 }}>
                <Typography sx={{ fontSize: '18px', fontWeight: 700, color: '#1e293b' }}>
                  {menuCandidate?.name || '-'}
                </Typography>
                <Typography sx={{ fontSize: '14px', fontWeight: 600, color: '#f59e0b', marginTop: '2px' }}>
                  {menuCandidate?.role || '-'}
                </Typography>
                <Typography sx={{ fontSize: '13px', color: '#64748b', marginTop: '2px' }}>
                  {menuCandidate?.experience || ''}
                </Typography>
              </Box>
              {menuCandidate?.status && (
                <Chip
                  size="small"
                  label={menuCandidate.status}
                  sx={{
                    textTransform: 'capitalize',
                    background: menuCandidate.status === 'active' ? 'rgba(16, 185, 129, 0.12)' : 'rgba(245, 158, 11, 0.12)',
                    color: menuCandidate.status === 'active' ? '#10b981' : '#f59e0b',
                    fontWeight: 700
                  }}
                />
              )}
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '12px', marginBottom: '16px' }}>
              <Box>
                <Typography sx={{ fontSize: '11px', color: '#64748b', textTransform: 'uppercase', fontWeight: 700 }}>
                  Department
                </Typography>
                <Typography sx={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px' }}>
                  {menuCandidate?.department || '-'}
                </Typography>
              </Box>
              <Box>
                <Typography sx={{ fontSize: '11px', color: '#64748b', textTransform: 'uppercase', fontWeight: 700 }}>
              ...
              <Box>
                ...
                <Typography sx={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px', wordBreak: 'break-word' }}>
                  {menuCandidate?.email || '-'}
                </Typography>
              </Box>
              <Box>
            ...
            <Typography sx={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase', fontWeight: 700, marginBottom: '8px' }}>
              Skills
            </Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {(menuCandidate?.skills || []).map((skill: string, idx: number) => (
                <Chip
                  key={`${skill}-${idx}`}
                  label={skill}
                  size="small"
        ...
        {/* Filter dialog */}
        <Dialog open={isFilterOpen} onClose={handleCloseFilter} maxWidth="sm" fullWidth>
          <DialogTitle sx={{ paddingBottom: '8px' }}>Filter Candidates</DialogTitle>
          <DialogContent dividers>
            <Typography sx={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase', fontWeight: 800, marginBottom: '8px' }}>
              Status
            </Typography>
            <FormGroup row sx={{ gap: '12px', marginBottom: '16px' }}>
              {['active', 'pending'].map((status) => (
                <FormControlLabel
                  key={status}
                  control={
                    <Checkbox
                      checked={Boolean((filters.statuses as Record<string, boolean>)?.[status])}
                      onChange={(e) =>
                        setFilters((prev) => ({
                          ...prev,
                          statuses: { ...(prev.statuses as Record<string, boolean> || {}), [status]: e.target.checked },
                        }))
                      }
                      sx={{
                        color: '#cbd5e1',
                  ...
                  label={<span style={{ textTransform: 'capitalize' }}>{status}</span>}
                />
              ))}
            </FormGroup>
            ...
            <Box sx={{ padding: '0 8px', marginBottom: '16px' }}>
              <Slider
                ...
                sx={{
                  color: '#f59e0b',
                  '& .MuiSlider-thumb': { boxShadow: '0 4px 12px rgba(245,158,11,0.35)' },
                }}
              />
            </Box>
            ...
            <FormGroup sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '6px' }}>
              {allDepartments.map((dept) => (
                <FormControlLabel
                  key={dept}
                  control={
                    <Checkbox
                      checked={Boolean((filters.departments as Record<string, boolean>)?.[dept])}
                      onChange={(e) =>
                        setFilters((prev) => ({
                          ...prev,
                          departments: { ...(prev.departments as Record<string, boolean> || {}), [dept]: e.target.checked },
                        }))
                      }
                      sx={{
                        color: '#cbd5e1',
  ...
  return (
    <Navigation>
      ...
      {/* All the dialogs and menus remain the same */}
      {/* Profile Dialog */}
      <Dialog 
        open={isProfileOpen} 
        ...
        {menuCandidate && (
          <>
            ...
            <DialogContent sx={{ padding: '24px 32px' }}>
              <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '24px', marginBottom: '32px' }}>
                <Avatar sx={{
                  width: 80,
                  height: 80,
                  background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)',
                  ...
                  {menuCandidate.name.charAt(0)}
                </Avatar>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="h5" sx={{ fontWeight: 700, color: '#1e293b', marginBottom: '8px' }}>
                    {menuCandidate.name}
                  </Typography>
                  <Typography sx={{ color: '#64748b', marginBottom: '8px' }}>
                    {menuCandidate.role}
                  </Typography>
                  <Typography sx={{ color: '#64748b', marginBottom: '16px' }}>
                    {menuCandidate.experience} • {menuCandidate.department}
                  </Typography>
                  <Box sx={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                    {menuCandidate?.skills?.map((skill: string, index: number) => (
                      <Chip
                        key={index}
                        label={skill}
                        size="small"
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <Chip
                      label={`Score: ${menuCandidate.score}%`}
                      sx={{
                        background: menuCandidate.score >= 80 ? '#dcfce7' : '#fef3c7',
                        color: menuCandidate.score >= 80 ? '#166534' : '#92400e',
                        fontWeight: 600
                      }}
                    />
                    <Chip
                      label={menuCandidate.status}
                      sx={{
                        background: menuCandidate.status === 'active' ? '#dcfce7' : '#fef3c7',
                        color: menuCandidate.status === 'active' ? '#166534' : '#92400e',
                        fontWeight: 600,
                        textTransform: 'capitalize'
                      }}
                    />
                ...
                <Box>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <i className="fas fa-envelope" style={{ color: '#64748b', width: '20px' }}></i>
                      <Typography>{menuCandidate.email}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <i className="fas fa-phone" style={{ color: '#64748b', width: '20px' }}></i>
                      <Typography>{menuCandidate.phone}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <i className="fas fa-calendar" style={{ color: '#64748b', width: '20px' }}></i>
                      <Typography>Hired: {menuCandidate.hireDate}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <Box sx={{
                        width: 12,
                        height: 12,
                        borderRadius: '50%',
                        background: menuCandidate.isOnline ? '#10b981' : '#94a3b8',
                        marginLeft: '4px'
                      }} />
                      <Typography sx={{ 
                        color: menuCandidate.isOnline ? '#10b981' : '#94a3b8',
                        fontWeight: 600
                      }}>
                        {menuCandidate.onlineStatus || (menuCandidate.isOnline ? 'Active' : 'Inactive')}
                      </Typography>
                      {menuCandidate.lastActivity && (
                        <Typography sx={{ fontSize: '12px', color: '#64748b' }}>
                          • Last seen: {new Date(menuCandidate.lastActivity).toLocaleString()}
                        </Typography>
                      )}
                    </Box>
                  </Box>
            ...
            <DialogActions sx={{ padding: '0 32px 32px 32px', gap: '12px' }}>
              <Button
                onClick={handleCloseProfile}
                sx={{
                  color: '#64748b',
      ...
      >
        ...
        <MenuItem 
          onClick={() => menuCandidate && handleGenerateQuestions(menuCandidate)} 
